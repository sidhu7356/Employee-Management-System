...
public class GlobalExceptionHandler {
    ...
    // ----------------------------------------------------------------
    // 400 – Invalid Sort Property / Property Reference Exception
    // ----------------------------------------------------------------
    @ExceptionHandler(PropertyReferenceException.class)
    public ResponseEntity<ErrorResponse> handlePropertyReferenceException(
            PropertyReferenceException ex, HttpServletRequest request) {
        String message = String.format("Invalid sort property: '%s' is not a valid field for '%s'",
    ...
    // ----------------------------------------------------------------
    // 400 – Malformed / Unreadable JSON Payload
    // ----------------------------------------------------------------
    @ExceptionHandler(HttpMessageNotReadableException.class)
    public ResponseEntity<ErrorResponse> handleHttpMessageNotReadable(
            HttpMessageNotReadableException ex, HttpServletRequest request) {
        log.warn("Malformed JSON payload: {}", ex.getMessage());
        String message = "Required request body is missing or malformed JSON payload";
        return buildResponse(HttpStatus.BAD_REQUEST, message, request.getRequestURI());
    }
